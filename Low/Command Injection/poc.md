...
- Command Injection의 핵심 원인은 **사용자 입력이 그대로 쉘 명령으로 실행되는 구조**입니다.  
따라서 대응의 핵심은 **입력이 절대 “명령”이 되지 못하도록 차단하는 것**입니다.

### 1. 입력값 화이트리스트 검증

- IP 입력창이라면 숫자(`0-9`)와 점(`.`)만 허용
- `;`, `&`, `|`, `&&`, `||`, `$()`, `` ` `` 등 쉘 메타문자 전면 차단
- 형식에 맞지 않으면 명령 실행 자체를 거부

---

### 2. 쉘 호출 함수 사용 금지

- `system()`, `exec()`, `shell_exec()` 등 쉘을 거치는 함수 사용 지양
- 가능하면 OS 명령 대신 내부 라이브러리 또는 API 사용
...
- 명령어는 서버에서 고정
- 사용자 입력은 인자로만 전달
- 문자열 결합 방식으로 명령 생성 금지

---

### 4. 최소 권한 원칙 적용

- 웹 서버 실행 계정을 root로 사용하지 않기
- 불필요한 파일 접근 권한 제거
- SELinux, AppArmor 등 보안 모듈 활용
