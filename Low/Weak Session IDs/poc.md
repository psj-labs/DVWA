...
- Weak Session ID의 핵심 문제는 **세션 값이 예측 가능하다는 것**입니다.  

---

### 1. PHP 기본 세션 기능 사용 (중요)

- 세션 ID를 직접 생성하지 않습니다.
- 반드시 PHP 내장 세션 메커니즘을 사용합니다.

```
session_start();
```

- PHP는 내부적으로 암호학적으로 안전한 난수 기반 세션 ID를 생성합니다.
...
- 단순 증가값, timestamp, mt_rand() 단독 사용은 예측 가능하므로 안전하지 않습니다.

---

### 3. 로그인 시 세션 재발급 (Session Regeneration)

- 로그인 성공 시 기존 세션을 폐기하고 새 세션을 발급합니다.
